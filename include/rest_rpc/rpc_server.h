    ...
    namespace rpc_service
    {
        ...
        public:
            ...
            void set_network_err_callback(
                std::function<void(std::shared_ptr<connection>, std::string /*reason*/)> on_net_err)
            {
        ...
        private:
            ...
            {
                conn_.reset(
                    new connection(io_service_pool_.get_io_service(), timeout_seconds_, router_));
                conn_->set_callback(
                    [this](std::string key, std::string token, std::weak_ptr<connection> conn) {
                        std::unique_lock<std::mutex> lock(sub_mtx_);
            ...
            std::function<void(std::shared_ptr<connection>, std::string)> on_net_err_callback_ =
                nullptr;
