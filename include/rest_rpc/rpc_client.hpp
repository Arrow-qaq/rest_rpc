...
namespace rest_rpc
{
    ...
    using subscribe_func = std::function<void(string_view)>;

    class rpc_client : private asio::noncopyable
    {
    public:
        rpc_client() : socket_(ios_), work_(std::make_shared<asio::io_context::work>(ios_)), deadline_(ios_), body_(INIT_BUF_SIZE)
        {
            thd_ = std::make_shared<std::thread>([this] { ios_.run(); });
        }

        rpc_client(client_language_t client_language, std::function<void(long, const std::string&)> on_result_received_callback)
            : socket_(ios_), work_(std::make_shared<asio::io_context::work>(ios_)), deadline_(ios_), body_(INIT_BUF_SIZE),
        ...
        rpc_client(client_language_t client_language, std::function<void(long, const std::string&)> on_result_received_callback, std::string host,
            unsigned short port)
            : socket_(ios_), work_(std::make_shared<asio::io_context::work>(ios_)), deadline_(ios_), host_(std::move(host)), port_(port),
              body_(INIT_BUF_SIZE), client_language_(client_language), on_result_received_callback_(std::move(on_result_received_callback))
        {
            thd_ = std::make_shared<std::thread>([this] { ios_.run(); });
        }
        ...
        void set_connect_timeout(size_t milliseconds) { connect_timeout_ = milliseconds; }

        void set_reconnect_count(int reconnect_count) { reconnect_cnt_ = reconnect_count; }
        ...
        void set_error_callback(std::function<void(asio::error_code)> f) { err_cb_ = std::move(f); }
        ...
        long internal_async_call(const std::string& encoded_func_name_and_args)
        {
            ...
            write(fu_id, request_type::req_res, std::move(sbuffer), MD5::MD5Hash32(encoded_func_name_and_args.data()));
            return (long)fu_id;
        }

        template<size_t TIMEOUT = DEFAULT_TIMEOUT, typename T, typename... Args>
        void async_call(const std::string& rpc_name, std::function<void(asio::error_code, T)> cb, Args&&... args)
        {
            if (!has_connected_)
            {
                if (cb)
                    cb(asio::error::make_error_code(asio::error::not_connected), "not connected");
                return;
            }
            ...
            uint64_t cb_id = 0;
            {
                ...
                const auto& func_callback = [this, cb, cb_id](asio::error_code code, string_view data) {
                    if (cb)
                    {
                        try
                        {
                            ...
                            {
                                std::unique_lock<std::mutex> lock(cb_mtx_);
                                if (callback_map_.find(cb_id) != callback_map_.end())
                                {
                                    callback_map_.erase(cb_id);
                                }
                            }
                        } catch (const std::exception&)
                        {
                            // 如果客户端退出，this会被释放，不能再使用this成员变量
                            cb(code, T());
                            return;
                        }
                    }
                ...
                auto call = std::make_shared<call_t>(ios_, func_callback, TIMEOUT);
                call->start_timer();
        ...
        // 重载的 subscribe 函数，使用模板参数推导
        template<typename T>
        void subscribe(std::string key, const std::function<void(const T&)>& f)
        {
            subscribe(key, [f](string_view data) { f(as_subscribe<T>(data)); });
        }

        template<typename T>
        void subscribe(const std::string& key, void (*f)(const T&))
        {
            subscribe(key, std::function<void(const T&)>(f));
        }

        template<typename T, typename C>
        void subscribe(const std::string& key, void (C::*f)(const T&), C* obj)
        {
            const auto& func = [f, obj](const T& t) { (obj->*f)(t); };
            subscribe<T>(key, func);
        }

        void subscribe(std::string key, std::string token, subscribe_func f)
        {
    ...
    private:
        void subscribe(std::string key, const subscribe_func& f)
        {
        ...
        void write(std::uint64_t req_id, request_type type, rpc_service::buffer_type&& message, uint32_t func_id)
        {
            ...
            client_message_type msg{
                req_id, type, {message.release(), size},
                  func_id
            };
        ...
        void call_back(uint64_t req_id, const asio::error_code& ec, string_view data)
        {
            {
                std::unique_lock<std::mutex> lock(cb_mtx_);
                if (future_map_.find(req_id) == future_map_.end() && callback_map_.find(req_id) == callback_map_.end())
                {
                    return;
                }
            }
            ...
            if (client_language_ == client_language_t::JAVA)
            {
                ...
                // handle error.
                on_result_received_callback_((long)req_id, std::string(data.data(), data.size()));
            } else
            {
                // For CPP client.
                temp_req_id_ = req_id;
        ...
        public:
            ...
            void start_timer()
            {
                ...
                timer_.async_wait([this, self](asio::error_code ec) {
                    ...
                    callback(asio::error::make_error_code(asio::error::timed_out), {});
                    has_timeout_ = true;
                });
            }
        ...
        std::unordered_map<std::string, subscribe_func> sub_map_;
